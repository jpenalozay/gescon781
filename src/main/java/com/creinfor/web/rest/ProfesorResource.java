...
import com.creinfor.service.ProfesorQueryService;
import com.creinfor.service.ProfesorService;
...
/**
 * REST controller for managing {@link com.creinfor.domain.Profesor}.
 */
@RestController
@RequestMapping("/api")
public class ProfesorResource {
    ...
    private final ProfesorQueryService profesorQueryService;

    public ProfesorResource(
        ProfesorService profesorService,
        ProfesorRepository profesorRepository,
        ProfesorQueryService profesorQueryService
    ) {
        this.profesorService = profesorService;
        this.profesorRepository = profesorRepository;
        this.profesorQueryService = profesorQueryService;
    }
    ...
    public ResponseEntity<Profesor> updateProfesor(
        ...
        Profesor result = profesorService.update(profesor);
        return ResponseEntity
            .ok()
    ...
    public ResponseEntity<List<Profesor>> getAllProfesors(ProfesorCriteria criteria) {
        log.debug("REST request to get Profesors by criteria: {}", criteria);
        List<Profesor> entityList = profesorQueryService.findByCriteria(criteria);
        return ResponseEntity.ok().body(entityList);
    }

    /**
     * {@code GET  /profesors/count} : count all the profesors.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/profesors/count")
    public ResponseEntity<Long> countProfesors(ProfesorCriteria criteria) {
        log.debug("REST request to count Profesors by criteria: {}", criteria);
        return ResponseEntity.ok().body(profesorQueryService.countByCriteria(criteria));
    }
